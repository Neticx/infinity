					...
					<div>
						...
						<div class="x_content">
							<div class="form-group">
		                        <input type="text" class="form-control" id="searchUser" onkeyup="searchUser()" placeholder="Cari Nama">
	                        </div>
						</div>
						<ul id="listUser" class="list-unstyled top_profiles scroll-view" style="overflow-y: scroll;">
							<?php foreach($users as $user): ?>
							<li class="media event" style="cursor: pointer;" onclick="doChat(<?php echo $user['id_user'].",'".$user['nama']."'"; ?>)">
								<a class="pull-left border-green profile_thumb">
									<i class="fa fa-user green"></i>
								</a>
								<div class="media-body">
									<a class="title"><?php echo $user['nama']; ?></a>
									<p><?php echo $user['otoritas']; ?><br />
									<small><?php echo $user['divisi']; ?></small></p>
								</div>
							</li>
							<?php endforeach; ?>
						</ul>
...
<script type="text/javascript">
	...
	function doChat(id, nama){
		strangerID = id;
		$.ajax({
			url: '<?php echo base_url('chat/get_chat/?stranger='); ?>'+strangerID,
			type: 'GET',
			dataType: 'html',
			success: function (data) {
				$('#namaStranger').empty();
	...
	function retrieveMessages() {
		if(strangerID != null){
	        $.get("<?php echo base_url('chat/get_message/?stranger='); ?>"+strangerID, function (data) {
	        	if(data != ''){
		            $('#bagianChat').append(data);
	...
	function sendMessage(){
		...
		if(pesan != ''){
			$.post("<?php echo base_url('chat/send_message/?stranger='); ?>"+strangerID,
				{
		          pesan: pesan,
		        },
		        function () {
