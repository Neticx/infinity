...
class Pengajuan extends MY_Controller {
	...
	*/
	...
	public function add_va()
	{
		...
		if($this->pengajuan->cekIdCabang($id)){
			...
			if($this->pengajuan->insertVirtualAccount($data, $data2)){
				$this->session->set_flashdata('alert_type', 'success');
				$this->session->set_flashdata('alert_data', 'Berhasil menambahkan data pengajuan pembayaran khusus');
				redirect(base_url('pengajuan/virtual_account/?cabangID='.$id));
			}
			else{
				$this->session->set_flashdata('alert_type', 'danger');
				$this->session->set_flashdata('alert_data', 'Gagal menambahkan data pengajuan pembayaran khusus');
				redirect(base_url('pengajuan/virtual_account/?cabangID='.$id));
			}
	...
	public function edit_va()
	{
		...
		if($this->pengajuan->cekIdCabang($id) && $this->pengajuan->cekIdVA($id2)){
			$cabang = $this->pengajuan->getDataCabang();
			$data = array(
				'title' => 'Edit Request Virtual Account',
				'cabang' => $cabang,
				'id_va' => $id2,
				'sub_req_va' => $this->pengajuan->getSubVA($id2),
			);
			$this->_render("pengajuan/edit_virtual_account",$data);
		}
		else{
			$this->error_404();
		}
	...
	public function delete_va()
	{
		...
		if($this->pengajuan->cekIdCabang($id)){
			if($this->pengajuan->deleteVA($id2)){
				$this->session->set_flashdata('alert_type', 'success');
				$this->session->set_flashdata('alert_data', 'Berhasil menghapus data pengajuan virtual account');
				redirect(base_url('pengajuan/daftar_virtual_account/?cabangID='.$id));
			}
			else{
				$this->session->set_flashdata('alert_type', 'danger');
				$this->session->set_flashdata('alert_data', 'Gagal menghapus data pengajuan pembayaran khusus');
	...
	public function action_edit_va()
	{
		...
		if($this->pengajuan->cekIdCabang($id) && $this->pengajuan->cekIdVA($id2)){
			$data = array();
			$i = 0;
			foreach($this->input->post('id_sub_req') as $id_sub){
				$data[] = array(
							'id_sub_req_va' => $id_sub,
							'virtual_account' => $this->input->post('va')[$i],
						);
				$i++;
			}

			if($this->pengajuan->updateSubVA($data)){
				$this->session->set_flashdata('alert_type', 'success');
				$this->session->set_flashdata('alert_data', 'Berhasil mengubah data pengajuan virtual account');
				redirect(base_url('pengajuan/edit_va/?cabangID='.$id.'&reqID='.$id2));
			}
			else{
				$this->session->set_flashdata('alert_type', 'danger');
				$this->session->set_flashdata('alert_data', 'Gagal mengubah data pengajuan pembayaran khusus');
				redirect(base_url('pengajuan/edit_va/?cabangID='.$id.'&reqID='.$id2));
			}
		}
		else{
	...
	public function list_data_va()
	{
		...
		if($this->input->is_ajax_request()){
			...
			$jumlah = $this->pengajuan->getListVATotal($id, $array['search']);
			$i = ($array['offset'] == 0 || $array['offset'] == null)?1:$array['offset']+1;
			header('Content-type: application/json');
			$dataJSON = array();
			$dataJSON['total'] = $jumlah;
			$dataJSON['rows'] = array();
			foreach($this->pengajuan->getListVA($id, $array) as $result){
				$data = array(
						'no' => $i,
						...
						'tahun' => $result->tahun,
						'status' => ($result->disetujui == '0')?'<span class="label label-danger">Belum Disetujui</span>':'<span class="label label-success">Disetujui</span>',
						'aksi' => "<a href=\"".base_url('cetak/request_pembayaran_khusus/?reqID='.$result->id)."\" target=\"_blank\" class=\"btn btn-xs btn-success\">Cetak</a> <a href=\"".base_url('pengajuan/edit_va/?cabangID='.$result->id_cabang.'&reqID='.$result->id)."\" class=\"btn btn-xs btn-primary\">Edit</a> <a onclick=\"doHapus($result->id,'$result->nomor - $result->tahun')\" class=\"btn btn-xs btn-danger\">Hapus</a>"
					);
				array_push($dataJSON['rows'], $data);
				$i++;
			}
