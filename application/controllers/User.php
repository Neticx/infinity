	...
	public function delete()
	{
		...
		if($this->user->cekIdUser($id)){
			if($this->user->deleteUser($id)){
				...
				$this->session->set_flashdata('alert_data', '<strong>SUCCESS. </strong> Berhasil menghapus user.');
				redirect(base_url('user'),'refresh');
			}
			else{
				$this->session->set_flashdata('alert_type', 'danger');
				$this->session->set_flashdata('alert_data', '<strong>ERROR. </strong> Gagal menghapus user.');
				redirect(base_url('user'),'refresh');
			}
		}
		else{
