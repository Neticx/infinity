	...
	*/
	...
	public function login()
	{
		...
		if($this->authValidation()){
			...
			if($this->auth->auth($email, $password, $tipe)){
				...
				if($tipe == 'siswa'){
					$this->authAlert = 'E-mail or Password tidak tersedia!! Mungkin anda belum melakukan registrasi, silahkan melakukan registrasi ke Aplikasi';
				}
				else{
	...
	{
		if($this->session->userdata('sess_user') == true && $this->session->userdata('nama_user') !== null && $this->session->userdata('role_user') !== null && $this->session->userdata('id_user') !== null){
			$array = array('id_user','sess_user','nama_user','role_user','foto_user','access_user','cabang_user');
